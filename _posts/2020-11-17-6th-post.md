---
layout: single
title: "[Netwhat] 문제 정리"
categories: "Netwhat"
tags: [Netwhat, C]
---

## 1. IP adress

선행지식

IPv4 (32bit)와 IPv6 (128bit)  
IP Class

### 1.1. IP주소 크기 구하기

Q. What is the size of an IPv4 address?

- 128 bits
- **32 bits**
- 64 miles
- 16 bits
- 8 bits
- 64 bytes
- 128 bytes

Q. What is the size of an IPv6 address?

- **128 bits**
- 32 bits
- 64 miles
- 16 bits
- 8 bits
- 64 bytes
- 128 bytes

Q. To the test IP stack on your local host, which IP address would you ping?

- 127.0.0.0
- 1.0.0.127
- **127.0.0.1**
- 127.0.0.255

> 로컬호스트의 주소는 127.0.0.1이다.

### 1.2. IP주소 클래스 구하기

Q. What is the default IP address class available?

- A AND B
- FIRST
- **C**
- B

Q. Which IP address class has more host addresses available by default?

- C
- D
- E
- F
- **FIRST(A)**

> A 클래스는 첫번째 옥텟을 제외한 나머지 3개의 옥텟이 모두 host ID이다.

Q. Which of this is not a class of IP address?

- Class D
- **Class F**
- Class C
- Class E

> IP 주소의 클래스는 A, B, C, D, E가 있다.

## 2. Subnetting

선행지식

기본 서브넷 마스크(Default Subnet Mask) 와 CIDR 표기법  
네트워크/브로드캐스트 주소 계산법  
호스트주소 수 계산법  
호스트주소 범위 (서브네팅 계산 방법)

### 2.1 서브넷마스크 표기법

Q. What is the CIDR notation of the 255.255.128.0 subnet mask?

- /8
- /16
- /9
- **/17**

> 255.255.128.0 을 비트로 표현하면 11111111.11111111.10000000.00000000 이다.  
> 따라서, 좌측에서 17비트가 Network ID라는 뜻인 /17이 맞다.

Q. What is the CIDR notation of the 255.255.192.0 subnet mask?

- /5
- /31
- **/18**
- /14

> 255.255.192.0 을 비트로 표현하면 11111111.11111111.11000000.00000000 이다.
> 따라서, 좌측에서 18비트가 Network ID라는 뜻인 /18이 맞다.

Q. What is the subnet mask of /24?

- 255.255.255.255
- **255.255.255.0**
- 255.0.0.0
- 255.255.128.0
- 255.192.0.0
- 255.224.0.0

> 좌측에서 24비트가 Network ID라는 뜻이므로, 서브넷마스크는 네트워크 ID 부분의 비트가 모두 1로 치환된 11111111.11111111.11111111.00000000, 즉 255.255.255.0 이다.

### 2.2. 네트워크/브로드캐스트 주소 구하기

Q. What is the network address of a host with an IP address of 182.161.121.118/24?

- 180.0.0.0
- 182.161.121.64
- 182.161.120.0
- 182.161.121.116
- 0.0.0.0
- 182.161.96.0
- 182.160.0.0
- **182.161.121.0**

> 네트워크 주소를 구하려면, IP주소와 서브넷마스크를 AND연산하면 된다.
> (182.161.121.118) & (255.255.255.0) = 182.161.121.0

Q. What is the network address of a host with an IP address of 107.212.146.212/25?

- 107.212.146.208
- 107.128.0.0
- 0.0.0.0
- 64.0.0.0
- 107.212.128.0
- **107.212.146.128**
- 107.212.0.0
- 107.208.0.0
- 107.212.146.192

> (107.212.146.212) & (255.255.255.128) = 107.212.146.128

Q. What is the network address of a host with an IP address of 166.175.144.121/23?

- 166.128.0.0
- **166.175.144.0**
- 166.175.144.96
- 128.0.0.0
- 166.174.0.0
- 166.0.0.0
- 166.0.0.0
- 166.175.144.120

> (166.175.144.121) & (255.255.254.0) = 166.175.144.0

Q. What is the network address of a host with an IP address of 116.45.224.50/8?

- 116.0.1.0
- **116.0.0.0**
- 116.255.255.0
- 116.255.255.255

> (116.45.224.50) & (255.0.0.0) = 116.0.0.0

Q. What is the network address of a host with an IP address of 45.195.37.187/16?

- 45.194.37.187
- 45.0.0.0
- 45.194.0.0
- **45.195.0.0**

> (45.195.37.187) & (255.255.0.0) = 45.195.0.0

Q. What is the broadcast address of a host with an IP address of 51.254.122.100/24?

- 51.254.122.0
- 51.254.122.1
- 51.254.122.254
- **51.254.122.255**

> 브로드캐스트 주소를 구하는 방법은, 서브넷마스크의 0인 부분을 모두 1로 바꾸는 것이다.
> 따라서, 51.254.122.100의 마지막 옥텟(서브넷마스크의 0인 부분)을 모두 1로 치환한 51.254.122.255가 된다.

### 2.3. 호스트 주소 수 구하기

Q. What is the maximum number of IP addresses that can be assigned to hosts on a local subnet using the 255.255.255.128 subnet mask?

- 128
- 60
- **126**
- 62
- 252
- 258
- 124
- 58
- 64

> 2^(32 - 25) - 2 = 128 - 2 = 126
> 255.255.255.0~255.255.255.127까지 총 128개가 될 수 있으나, 255.255.255.0은 네트워크 주소, 255.255.255.255는 브로드캐스트 주소로 쓰기 때문에 사용하면 안된다. 따라서 128-2=126개의 호스트를 할당할 수 있다.

Q. What is the maximum number of IP addresses that can be assigned to hosts on a local subnet using the 255.224.0.0 subnet mask?

- 4194306
- 4194302
- 1048578
- 4194300
- 2097148
- 4194298
- 1048574
- **2097150**
- 1048576

> 2^(32 - 11) - 2 = 2097152 - 2 = 2097150

Q. You have an interface on a router with the IP address of 124.144.156.248/21. Including the router interface, how many hosts can have IP addresses on the local network connected to the router interface?

- 1020
- 2050
- 2044
- 4090
- **2046**
- 2048
- 4092
- 2042
- 4094

> 2^(32 - 21) - 2 = 2048 - 2 = 2046

Q. You have an interface on a router with the IP address of 240.19.3.205/12. Including the router interface, how many hosts can have IP addresses on the local network connected to the router interface?

- 1048576
- 2097154
- **1048574**
- 524284
- 1048578
- 2097148
- 1048572

> 2^(32 - 12) - 2 = 1048576 - 2 = 1048574

### 2.4. 호스트주소 범위 구하기

Q. Which of the following proposals is the valid host range for the subnet on which the IP address 158.167.18.156/15 resides?

- 158.166.0.1- 158.167.255.253
- 158.165.255.253- 158.167.255.254
- **158.166.0.1- 158.167.255.254**
- 158.166.0.2- 158.168.0.2

> (Network)158.166.0.0 (Broadcast) 158.167.255.255

Q. Which of the following proposals is the valid host range for the subnet on which the IP address 1.93.149.6/17 resides?

- 1.93.127.255- 1.93.255.250
- 1.93.128.1- 1.94.0.1
- 1.93.128.1- 1.93.255.251
- **1.93.128.1- 1.93.255.254**
- 1.93.128.1- 1.94.0.3

> (Network)1.93.128.0 (Broadcast) 1.93.255.255

## 3. Public/Private IP address

선행지식

사설IP 주소대역

### 3.1. 사설 IP주소 고르기

Q. Which of the following IP addresses is a private address?

- 169.153.119.123
- 24.23.102.151
- 255.62.136.173
- **10.166.25.20**
- 46.244.138.171
- 27.147.158.251

> 사설 IP의 주소는 다음 3가지 주소대역으로 고정된다.
>
> - A 클래스: **10**.0.0.0 ~ **10**.255.255.255
> - B 클래스: **172.16**.0.0 ~ **172.31**.255.255
> - C 클래스: **192.168**.0.0 ~ **192.168**.255.255

Q. Which of the following IP addresses is a private address?

- 222.9.230.144
- 135.167.134.35
- 172.32.0.5
- 27.157.141.96
- **172.16.0.2**
- 129.244.78.149
- 137.223.167.235

Q. Which of the following IP addresses is a private address?

- 108.246.233.231
- 146.227.105.173
- 59.155.254.18
- 253.29.133.220
- **192.168.20.253**
- 94.152.104.99

## 4. Transport Layer

선행지식

OSI 7 Model  
TCP / UDP 원리 및 차이

### 4.1. TCP/UDP 특징

Q. Which of the following propositions is not true?

- UDP is faster, simpler and more efficient than TCP
- UDP only has the basic error control mechanism
- UDP is a datagram oriented protocol
- **UDP does not support broadcasting**

> TCP는 일 대 일(unicasting)만 지원하는 반면, UDP는 일 대 일(unicasting) 일 대 다(broadcasting), 다 대 다(multicasting) 모두 지원한다.

Q. Which of the following propositions is not true?

- TCP is a connection-oriented protocol
- TCP does not support broadcasting
- TCP provides extended error checking mechanisms, because it provides flow control and data acknowledgement
- Data sequencing is a TCP feature (this means that packets arrive in order in the recipient)
- **The delivery of data to the destination cannot be guaranteed in TCP**
- TCP is reliable because it guarantees the delivery of data to the router of the destination

> TCP는 신뢰성 있는 데이터 전송이 장점이다. 데이터의 재전송을 보장한다.

Q. Which of the following propositions is not true?

- UDP is faster, simpler and more efficient than TCP
- **UDP provides extended error checking mechanisms, because it provides flow control and data acknowledgement**
- UDP is a datagram oriented protocol
- UDP supports broadcasting

> UDP는 flow control(흐름제어)을 지원하지 않는다.

Q. Which of the following propositions is not true?

- **TCP is a datagram oriented protocol**
- TCP does not support broadcasting
- TCP provides extended error checking mechanisms, because it provides flow control and data acknowledgement
- Data sequencing is a TCP feature (packets arrive in order in the recipient)
- TCP is reliable because it guarantees the delivery of data to the router of the destination
- TCP is comparatively slower than UDP

> TCP는 byte-stream을 통한 연결이다. UDP가 datagram을 통한 연결이다.

Q. Which of the following services use UDP? 1. DHCP 2. SMTP 3. FTP 4. HTTP

- 3
- **1**
- 2
- All of the above

> DHCP 프로토콜은 UDP를 이용한다.

## 5. Application Layer

선행지식

OSI 7 Model  
[DHCP/DNS 개념](DHCP/DNS 개념)

### 5.1. DNS/DHCP 특징

Q. \_\_\_\_ translates Internet domain names and host names into IP addresses

- Network time protocol
- Default routing protocol
- **Domain name system**
- OSI model system

> DNS(Domain Name System)은 도메인 이름을 IP 주소로 변환시켜주는 역할을 한다.

Q. You want to implement a mechanism that automates IP configuration, including IP address, subnet mask, default gateway and DNS information. What protocol will you use to achieve this?

- SNMP
- **DHCP**
- SMTP
- ARP

> DHCP 프로토콜은 동적 호스트 설정 프로토콜로서 해당 호스트에게 'IP주소, 서브넷 마스크, 기본 게이트웨이 IP 주소, DNS서버 IP주소'를 자동으로 일정 시간 할당해주는 인터넷 프로토콜이다.

Q. What DHCP protocol does it use at the transport layer level?

- ICMP
- TCP
- FTP
- **UDP**

Q. What type of address is supported by DHCP?

- IPV4
- IPV6
- **IPV4 and IPV6**
- None of them

Q. DHCP is used for \_\_\_

- IPV4
- **Both IPV6 and IPV4**
- IPV6
- None of the mentioned

* 10분만에 대략 30개문항을 풀어야하기 때문에, 계산하는 모든 문제들은 http://jodies.de/ipcalc?host=&mask1=255.252.0.0&mask2= 와 같은 IP 계산기를 이용하는 것을 권장한다.
