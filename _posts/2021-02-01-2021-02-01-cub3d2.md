...
- 리턴값: 이미지가 저장된 메모리 영역의 시작을 나타내는 char \*를 리턴한다.
  ...
  - 두번째 그룹의 bits_per_pixel 비트는 첫 줄의 두번째 픽셀을 나타내고 그런식으로 쭉 나간다.
...
# 간단한 실습: MiniLibX를 이용해 창을 띄우고, 픽셀을 그리고, ESC로 종료시키기
...
int	main()
{
	...
	mlx_ptr = mlx_init();
	win_ptr = mlx_new_window(mlx_ptr, 500, 500, "띄울_창_타이틀");
	mlx_pixel_put(mlx_ptr, win_ptr, 250, 250, 0xFFFFFF); //정 가운데 흰색 픽셀(점) 찍기
	mlx_loop(mlx_ptr);
}
```
