...
- C 프로그래밍의 매우 흥미로운 개념인 static variable(정적 변수)에 대해 학습한다.
<br>
...
이 파트를 위해 모든 initial files 이름들에 _bonus를 붙인 파일들을 제출해야 한다.

•	하나의 정적 변수(static variable)로 get_next_line 성공한다.

•	get_next_line을 사용하여 다중 file descriptor를 관리 할 수 있다.
예를 들어, file descriptor 3, 4, 5가 읽을 수 있는 경우, get_next_line은 3에서 한 번, 4에서 한 번, 다시 3에서 한 번, 5에서 한 번 호출할 수 있다. (각 descriptor의 reading thread를 잃지 않으면서)
